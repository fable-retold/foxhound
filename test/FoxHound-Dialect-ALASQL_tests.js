				...
				(
					...
					function()
					{
						...
						Expect(testFoxHound.dialect.name)
							.to.equal('ALASQL');
						Expect(testFoxHound)
							.to.be.an('object', 'FoxHound with ALASQL should initialize as an object directly from the require statement.');
					}
				);
			}
		);
			...
			function()
			{
				...
				(
					...
					function()
					{
						...
						_Fable.log.trace('Select Query', tmpQuery.query);
						Expect(tmpQuery.query.body)
							.to.equal('SELECT `Name`, `Age`, `Cost` FROM Animal WHERE `Deleted` = :Deleted_w0 ORDER BY `IDAnimal` LIMIT 100;');
					}
				);
				test
				(
