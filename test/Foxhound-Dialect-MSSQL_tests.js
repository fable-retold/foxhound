			...
			function()
			{
				...
				(
					...
					function()
					{
						...
						_Fable.log.trace('Select Query', tmpQuery.query);
						Expect(tmpQuery.query.body)
							.to.equal('SELECT DISTINCT [Name], [Age], [Cost] FROM [Animal] WHERE [Deleted] = @Deleted_w0 ORDER BY (SELECT 1) OFFSET 0 ROWS FETCH NEXT 100 ROWS ONLY;');
					}
				);
				test
				(
				...
				(
					...
					function()
					{
						...
						Expect(tmpQuery.query.body)
							.to.equal('SELECT [Name], [Age], [Cost] FROM (SELECT [Name], [Age], [Cost], ROW_NUMBER() OVER (ORDER BY [Age], [IDAnimal]) AS [_RowNum] FROM [Animal] WHERE [Deleted] = @Deleted_w0) AS [_Paged] WHERE [_RowNum] > 20 AND [_RowNum] <= 30;');
					}
				);
				test
				(
			...
			function()
			{
				...
				(
					...
					function()
					{
						...
						Expect(tmpQuery.query.body).to.contain('ORDER BY (SELECT 1)');
					}
				);
				test
				(
