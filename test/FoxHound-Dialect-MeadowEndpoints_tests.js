			...
			function()
			{
				...
				(
					...
					function()
					{
						...
						_Fable.log.trace('Select Query', tmpQuery.query);
						Expect(tmpQuery.query.body)
							.to.equal('Animals/LiteExtended/Name,Age,Cost/FilteredTo/FBV~Age~EQ~25~FOP~0~(~0~FBV~Color~EQ~Red~FBVOR~Color~EQ~Green~FCP~0~)~0~FBV~Description~NN~0~FBL~IDOffice~INN~10,11,15,18,22~FSF~Age~ASC~0/0/100');
					}
				);
				test
				(
					'IN list emits a list stanza, not a value stanza',
					function()
					{
						// FBV never splits its value on commas, so an FBV
						// carrying "10,11,15" arrives at the remote as one
						// scalar string and a numeric column coerces it to
						// its leading integer — the list silently collapses
						// to its first element. Only FBL/FBLOR survive the
						// round trip.
						var tmpQuery = libFoxHound.new(_Fable)
							.setDialect('MeadowEndpoints')
							.setScope('Animal')
							.addFilter('IDOffice', [10, 11, 15], 'IN');
						tmpQuery.buildReadQuery();
						Expect(tmpQuery.query.body)
							.to.equal('Animals/FilteredTo/FBL~IDOffice~INN~10,11,15');
					}
				);
				test
				(
					'OR-connected IN list emits FBLOR',
					function()
					{
						var tmpQuery = libFoxHound.new(_Fable)
							.setDialect('MeadowEndpoints')
							.setScope('Animal')
							.addFilter('Color', 'Red')
							.addFilter('IDOffice', [10, 11], 'IN', 'OR');
						tmpQuery.buildReadQuery();
						Expect(tmpQuery.query.body)
							.to.equal('Animals/FilteredTo/FBV~Color~EQ~Red~FBLOR~IDOffice~INN~10,11');
					}
				);
				test
				(
					'NOT IN emits NIN, the operator the filter grammar recognizes',
					function()
					{
						// 'NI' is not in the grammar; the remote's parser falls
						// through to '=' on an unknown operator, turning an
						// exclusion into an equality.
						var tmpQuery = libFoxHound.new(_Fable)
							.setDialect('MeadowEndpoints')
							.setScope('Animal')
							.addFilter('IDOffice', [10, 11], 'NOT IN');
						tmpQuery.buildReadQuery();
						Expect(tmpQuery.query.body)
							.to.equal('Animals/FilteredTo/FBL~IDOffice~NIN~10,11');
					}
				);
				test
				(
					'a scalar IN value still emits a well-formed list stanza',
					function()
					{
						var tmpQuery = libFoxHound.new(_Fable)
							.setDialect('MeadowEndpoints')
							.setScope('Animal')
							.addFilter('IDOffice', 10, 'IN');
						tmpQuery.buildReadQuery();
						Expect(tmpQuery.query.body)
							.to.equal('Animals/FilteredTo/FBL~IDOffice~INN~10');
					}
				);
				test
				(
			...
			function()
			{
				...
				(
					...
					function()
					{
						...
						_Fable.log.trace('Select Query', tmpQuery.query);
						Expect(tmpQuery.query.body)
							.to.equal('Animals/LiteExtended/Name,Age,Cost/FilteredTo/FBV~Age~EQ~25~FOP~0~(~0~FBV~Color~EQ~Red~FBVOR~Color~EQ~Green~FCP~0~)~0~FBV~Description~NN~0~FBL~IDOffice~INN~10,11,15,18,22~FBV~Deleted~EQ~1/0/100');
					}
				);
				test
				(
